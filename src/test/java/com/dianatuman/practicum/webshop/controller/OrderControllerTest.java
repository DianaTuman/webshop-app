...
public class OrderControllerTest extends BaseControllerTest {
    ...
    public void getOrders_shouldReturnOrdersPage() {
        when(orderService.getOrders()).thenReturn(Flux.just(new OrderDTO(1L, Flux.just())));
        webTestClient.get()
                .uri("/orders")
                .exchange()
                .expectStatus().isOk()
    ...
    public void getOrdersId_shouldReturnOrderPage() {
        ...
        e1.setCount(1);
        e2.setCount(1);
        when(orderService.getOrder(1)).thenReturn(Mono.just(new OrderDTO(1L, Flux.just(e1, e2))));
